...
// Define default bitrate settings for i.MX35ADS RINGO board
...
/* Bitrate table for PLL 66.5 MHz, desired Sample Point at 87.5% */
#define CAN_PRESDIV_50K_PLL			0x5E
#define CAN_PRESDIV_125K_PLL		0x25
#define CAN_PRESDIV_250K_PLL		0x12
	...
	while(optind < argc)
	{
		...
		while((opt = getopt(argc, argv, "ab:B:c:Di:l:m:Mn:psStu:vwxz")) != -1)
		{
			...
			case 'c':
				...
				else if(strncmp(optarg, "66M", 3) == 0)
