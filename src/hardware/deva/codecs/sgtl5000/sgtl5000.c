...
sgtl5000_parse_options (MIXER_CONTEXT_T * sgtl5000, char * args)
{
	...
	while (*args != '\0')
	{
		...
		switch (opt) {
			case OPT0:
				if (value == NULL) 
					ado_error ("SGTL5000: Must specify an I2C device number");
				else {
					strncpy (sgtl5000->i2c_dev, "/dev/i2c",MAX_STRLEN);
					strncat (sgtl5000->i2c_dev, value, MAX_STRLEN - strlen (sgtl5000->i2c_dev) );
				}
				break;
			case OPT1:
				if (value == NULL) 
					ado_error ("SGTL5000: Must specify a value to adr0cs");
				else
					sgtl5000->adr0cs = strtoul (value, NULL, 0);
				break;
			case OPT2:
				if (value == NULL) 
					ado_error ("SGTL5000: Must specify a value to mclk");
				else 
			...
			case OPT3:
				...
				printf("Example: io-audio -d [driver] mixer=i2cdev=0:adr0cs=0:mclk=1228800\n");
				printf("The driver now starts with default mixer options\n");
...
sgtl5000_mixer (ado_card_t * card, ado_mixer_t ** mixer, char * args, ado_pcm_t * pcm1)
{
	...
	strncpy (sgtl5000->i2c_dev, "", MAX_STRLEN);
	sgtl5000->adr0cs = -1;
	sgtl5000->samplerate = -1;
	sgtl5000->mclk = -1;
	...
	/* check i2c_dev option */
	if (strcmp (sgtl5000->i2c_dev, "") == 0)
	{
		ado_error ("SGTL5000: Missing i2cdev option, use default 0 for /dev/i2c0");
		strncpy (sgtl5000->i2c_dev, "/dev/i2c0", MAX_STRLEN);
	}

	/* check adr0cs option */
	if (sgtl5000->adr0cs != 0 && sgtl5000->adr0cs != 1)
	{
		ado_error ("SGTL5000: Invalid value for adr0cs \"%s\", must be 0 or 1, use default 0",
			sgtl5000_opts[OPT1]);
		sgtl5000->adr0cs = 0;
	}
	...
	if (sgtl5000->mclk == -1)
	{
		ado_error ("SGTL5000: Need to specify mclk (SYS_MCLK), use default 12288000");
		sgtl5000->mclk = 12288000;
	}
	else if ((sgtl5000->mclk < MCLK_MIN) || (sgtl5000->mclk > MCLK_MAX))
	{
		ado_error ("SGTL5000: mclk (SYS_MCLK) must be between 8M to 27M, use default 12288000");
		sgtl5000->mclk = 12288000;
	}
	...
	sgtl5000->i2c_fd = open (sgtl5000->i2c_dev, O_RDWR);
	if (sgtl5000->i2c_fd == -1)
	{
		ado_error ("SGTL5000 I2C_INIT: Failed to open I2C device %s", sgtl5000->i2c_dev);
		ado_free (sgtl5000);
		return (-1);
	}
