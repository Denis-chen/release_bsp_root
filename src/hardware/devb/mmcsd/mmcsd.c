	...
	if (ext->version >= MMC_VERSION_1) {	// MMC card
		...
		if (hba->verbosity)
			slogf(_SLOGC_SIM_MMC, _SLOG_INFO,
				"MMC config: clock %d, bus width %d, block size %d", 
				clock, 1, MMC_DFLT_BLKSIZE);
		...
		if (hba->verbosity)
			slogf(_SLOGC_SIM_MMC, _SLOG_INFO,
					"MMC : actual operation clock %d", clock);
...
static int mmc_ident_mmc_card_powerup(SIM_HBA *hba, int intr)
{
	...
	return mmc_sendcmd(hba, MMC_SEND_OP_COND, MMC_RSP_R3, MMC_CMD_INTR, ext->istage > 1 ? 0x00ff8000 : 0);
}
...
__SRCVERSION("mmcsd.c $Rev: 153421 $");
