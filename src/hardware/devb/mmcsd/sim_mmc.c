	...
	if (ext->nbytes == ext->offset) {	// All done
		if (ccb->cam_ch.cam_flags & CAM_SCATTER_VALID) {
			if (ext->dir == MMC_DIR_IN) {
				...
				uint32_t	sgi;
				uint8_t		*ptr = (uint8_t *)ext->bounce;

				for (sgi = 0; sgi < ccb->cam_sglist_cnt; sgi++, sge++) {
					memcpy((void *)sge->cam_sg_address, ptr, sge->cam_sg_count);
					ptr += sge->cam_sg_count;
				}
			}
		}
...
static inline void mmc_dma_initiate(SIM_HBA *hba)
{
	...
	if (ccb->cam_ch.cam_flags & CAM_SCATTER_VALID) {
		...
		for (sgi = 0; sgi < ccb->cam_sglist_cnt; sgi++, sge++) {
			nbytes += sge->cam_sg_count;
			if (nbytes > MMCSD_BOUNCE_SIZE)
				return mmc_error(hba, CAM_PROVIDE_FAIL);

			if (ext->dir == MMC_DIR_OUT) {
				memcpy(ptr, (void *)sge->cam_sg_address, sge->cam_sg_count);
				ptr += sge->cam_sg_count;
			}
		}

		paddr = (paddr_t)ext->pbounce;
	} else {
		nbytes = ccb->cam_dxfer_len;
		paddr = mmc_mphys(ccb, ccb->cam_data.cam_data_ptr, 0);
	}
	...
	if (mmc_card_ident(hba, intr) == MMC_SUCCESS) {
		if (ext->ident == MMCSD_IDENT_NONE) {	// ident complete
			...
			if(!(ext->eflags & MMC_EFLAG_READY)){
				...
				MsgSendPulse(mmc_ctrl.coid, SIM_PRIORITY, MMCSD_PULSE_CARD_READY, 0);
			}
			if (ccb)
				mmc_io_cmds(hba);
		}
...
//			atomic_set(&ext->eflags, MMC_EFLAG_MEDIA_CHANGED);
			...
			MsgSendPulse(mmc_ctrl.coid, SIM_PRIORITY, MMCSD_PULSE_CARD_INSERTION, 0);
		}
	} else {
		if (atomic_clr_value(&ext->eflags, MMC_EFLAG_INSERTED) & MMC_EFLAG_INSERTED) {
			...
			ext->powerdown(hba);	// powerdown the card
			MsgSendPulse(mmc_ctrl.coid, SIM_PRIORITY, MMCSD_PULSE_CARD_REMOVAL, 0);
		}
	}
}
...
{ 
	...
	if (hba->cfg.NumIRQs) {
		SIGEV_PULSE_INIT(&event, hba->coid, 21, SIM_INTERRUPT, NULL);
		...
		if (hba->cfg.NumIRQs > 1) {
			SIGEV_PULSE_INIT(&event, hba->coid, 21, SIM_DMA_INTERRUPT, NULL);
...
__SRCVERSION("sim_mmc.c $Rev: 153421 $");
