...
int	omap3_attach(SIM_HBA *hba)
{
	...
	// Capability
	ext->hccap |= MMC_HCCAP_ACMD12 | MMC_HCCAP_BW1 | MMC_HCCAP_BW4;	// 1 bit / 4 bits bus supported
	...
	if (!cfg->Description[0])
		strncpy(cfg->Description, "TI OMAP3 MMCHS", sizeof(cfg->Description));
...
__SRCVERSION("sim_omap3.c $Rev: 140019 $");
