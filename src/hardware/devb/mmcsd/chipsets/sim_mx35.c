...
static int mx35_reset_line(mx35_ext_t *mx35, uint32_t bits)
{
    ...
    out32(base + MX35_MMCWATML, ((0x80 << MMCWATML_WRWMLSHIFT) | (0x80 << MMCWATML_RDWMLSHIFT)));
	...
	if (mask & (MMCINTR_BRR | MMCINTR_BWR)) {	// PIO
		...
		while (1) {
			if (sts & maskerrsts) {
				...
				if (sts & MMCINTR_DCE){
		    		intr |= MMC_ERR_DATA_CRC;
					if (!mx35->sdcken){ //To1 need the workaround to re-indentify the card
...
static int mx35_clock(SIM_HBA *hba, int *clock, int high_speed)
{
	...
	*clock = max_clk / pre_div;
	...
	delay(1);
	out32(base + MX35_MMCSYSCTL, sysctl | 0x07 | mx35->sdcken);
	delay(10);
...
int	mx35_attach(SIM_HBA *hba)
{
	...
	// Capability
	ext->hccap |= MMC_HCCAP_BW8 | MMC_HCCAP_BW4 | MMC_HCCAP_BW1;	// 1,4 bnd 8 its bus supported
...
/*__SRCVERSION("sim_mx35.c $Rev: 140019 $");*/
