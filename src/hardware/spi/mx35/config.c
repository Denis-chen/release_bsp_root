...
int mx35_cfg(void *hdl, spi_cfg_t *cfg)
{
	mx35_cspi_t	*mx35 = hdl;
    ...
    ctrl = (ctrl-1) << CSPI_CONREG_BCNT_POS;

    for(i=0; i < 8 ; i++) {
        div = 4 << i;
        drate = mx35->clock / div;
		...
		if (drate <= cfg->clock_rate || i==7) {
			...
			ctrl |= i << CSPI_CONREG_DRATE_POS;
			break;
		}
    }

	if (!(cfg->mode & SPI_MODE_CKPOL_HIGH))
		ctrl |= CSPI_CONREG_POL;	/* Active low polarity */

	if (cfg->mode & SPI_MODE_CKPHASE_HALF)
		ctrl |= CSPI_CONREG_PHA;	/* Phase 1 */

	if (cfg->mode & SPI_MODE_CSPOL_HIGH)
		ctrl |= CSPI_CONREG_SSPOL;	/* SS polarity : active high */

	switch (cfg->mode & SPI_MODE_RDY_MASK) {
		case SPI_MODE_RDY_EDGE:
			ctrl |= CSPI_CONREG_DRCTL_EDGE << CSPI_CONREG_DRCTL_POS;
			break;
		case SPI_MODE_RDY_LEVEL:
			ctrl |= CSPI_CONREG_DRCTL_LEVEL << CSPI_CONREG_DRCTL_POS;
			break;
	}
